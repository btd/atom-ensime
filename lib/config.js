...
function processSExpression(expr, schema) {
  if(expr == null) {
    return expr;
  }
  ...
  if(schema.type === 'plist') {
    ...
    for(let i = 0; i < expr.length; i += 2) {
      ...
      obj[key] = processSExpression(value, schema.props[skey]);
    }
    return obj;
    ...
    return expr.map(function(ex) {
      return processSExpression(ex, schema.elems);
    });
  }
}
...
function parseEnsimeConfig(str) {
  ...
  return processSExpression(expr, configSchema);
}
